      ...
      try {
        speechUnlisten = await listen("speech-detected", async (event) => {
          try {
            ...
            if (!selectedSttProvider.provider && !useWhisprAPI) {
            ...
            if (!providerConfig && !useWhisprAPI) {
  ...
  const processWithAI = useCallback(
    async (
      ...
      try {
        ...
        const useWhisprAPI = await shouldUseWhisprAPI();
        if (!selectedAIProvider.provider && !useWhisprAPI) {
        ...
        try {
          for await (const chunk of fetchAIResponse({
            provider: useWhisprAPI ? undefined : provider,
