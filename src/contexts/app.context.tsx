...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Whispr API State
  const [whisprApiEnabled, setWhisprApiEnabledState] = useState<boolean>(
  ...
  const loadData = () => {
    ...
    // Load Whispr API enabled state
    const savedWhisprApiEnabled = safeLocalStorage.getItem(
      STORAGE_KEYS.WHISPR_API_ENABLED
    );
    if (savedWhisprApiEnabled !== null) {
      setWhisprApiEnabledState(savedWhisprApiEnabled === "true");
    }
  };
  ...
  const setWhisprApiEnabled = (enabled: boolean) => {
    setWhisprApiEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.WHISPR_API_ENABLED, String(enabled));
    loadData();
  };
